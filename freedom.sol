...
contract Freedom is ERC20, Ownable {
    ...
    bool public projectStarted = false; // each project shall start with at least one - hopefully trustworthy - person  
    ...
    event LOGAddress(address); // whenever we want to log any text, we can emit this event
    ...
    constructor() ERC20("Freedom", "FREE") {
        ...
        // invitedMembersOfTrustedInitiationCommunity[1] = 0x9E972a43B3B8D68cD70930697E16429E47E88151; // e.g. the developer
        invitedMembersOfTrustedInitiationCommunity[1] = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4; // e.g. the developer
    }
    ...
    function approveFreedomFan(address freedomFan) public avoidDuplicateApprovals(freedomFan) onlyNotYetApprovedFreedomFansCanBeApproved(freedomFan) {
        require(freedomFans[ids[msg.sender]].approvedOn > 0 || (projectStarted == false && owner() == msg.sender), "only approved freedom fans or the owner at the beginning can do that.");
    ...
    function startProject() public { 
        require(projectStarted == false && msg.sender == owner(), "startProject can only be executed once by the owner. at the end of startProject this contract is automatically renounced.");
